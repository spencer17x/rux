...
Current release limits are explicit: Rux Native secret Connection management stays Electron Main-owned, and SSH/reconnect/load testing is not complete. The Host advertises protocol v19; incompatible versions visibly block Runs, and Custom-Agent Runs pin an immutable Agent Revision plus a non-secret Provider Connection reference. Session Connector, isolated Handoff, local-data lifecycle, branch/worktree operations, and Run-owned review methods are exposed through confirmation-gated TUI commands. Task selection is session-local because the shared snapshot still has no separate active-Task field; restart deterministically selects the newest non-archived Task and shows history when more than one exists.

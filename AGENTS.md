...
- Runtime protocol v16 exposes confirmation-gated stdio methods for explicit Session import/refresh/revision, Context Handoff, local-data lifecycle/export, Git branches and Run-owned Changes. The Rust TUI consumes those methods for the Desktop-equivalent workflows that do not require Electron Main secrets. Rux Native credential management remains Desktop-only because `safeStorage` decryption is never delegated to the standalone Host.
- The final protocol-v16 macOS QA package has passed native Workspace selection, explicit official CLI detection, editable Composer, real Codex `QA_OK` completion, actual model/Token display, Agent selector discovery, and fail-closed unsigned update settings. It remains an ad-hoc QA bundle, not a signed production release.

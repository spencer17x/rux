...
- RUX includes a real `rux-native` Responses-compatible Adapter. It can read/list/write bounded UTF-8 files, stream Assistant deltas, and on macOS execute a structured no-shell command tool inside a restrictive `sandbox-exec` policy. Explicit Connection tests now save only Provider-returned `/models` entries and explicitly reported capabilities; Agent, Composer and Auto consume that same-Connection catalog. Custom headers and native OAuth are not yet implemented.
- The account surface manages Rux Native, Codex, and Claude Code Connections. It never inspects CLI login state on startup or panel open. Rux Native supports metadata edit, explicit Key replacement and credential deletion through Main-owned, fingerprinted Agent/Task impact previews; stale previews are rejected. API keys remain OS-encrypted and Renderer-inaccessible, and Providers are contacted only on explicit test or Run. CLI-owned credentials remain CLI-owned.

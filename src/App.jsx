  ...
  return (
    <div className="settings-shell">
      ...
      <main className="settings-content"><h1>模型与连接</h1>
        <section className="settings-section"><h2>GPT OAuth</h2><div className="settings-group oauth-group"><div className="settings-row provider-row"><div className="provider-mark"><OpenAiLogo size={27} weight="regular" /></div><strong>Codex CLI</strong><span className={auth.connected ? "connected-badge" : "disconnected-badge"}>{auth.connected ? "已连接" : "未登录"}</span><span className="provider-email">{auth.connected ? "使用 ChatGPT 账户登录" : auth.message || "未检测到本机 ChatGPT 登录"}</span><button type="button" className="secondary-button" onClick={() => execute(async () => { await onLogin(); return "已启动设备登录"; })}>{auth.connected ? "重新登录" : "登录"}</button></div><button type="button" className="danger-link disconnect-link" onClick={() => { if (window.confirm("确认退出本机 Codex 登录？")) execute(onLogout); }}>断开连接</button></div></section>

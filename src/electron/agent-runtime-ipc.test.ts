...
function harness(provider: "codex" | "custom" = "codex") {
...
describe("agent runtime IPC", () => {
  it("routes Claude turns and interrupts to the Claude adapter", async () => {
    const test = harness(); await test.invoke("agent:start", { runId: "run", agentId: "claude-code", projectId: "project", prompt: "hello" }); await test.invoke("agent:interrupt", { agentId: "claude-code", runId: "run", threadId: "thread", turnId: "turn" });
    expect(test.claudeClient.startTurn).toHaveBeenCalled(); expect(test.claudeClient.interrupt).toHaveBeenCalledWith("run");
  });

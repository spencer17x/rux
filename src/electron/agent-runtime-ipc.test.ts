...
function harness(provider: "codex" | "custom" = "codex", customSend: (input: any) => Promise<any> = async () => ({ text: "custom" }), inputModalities = ["text", "image"]) {
  ...
  registerAgentRuntimeIpc(ipc, { getWindow: () => ({ webContents: { send: (_channel: string, event: any) => emitted.push(event) } }) as any, settingsStore: { load: async () => ({ provider, model: "model", reasoning: "medium", sandboxMode: "workspace-write" }) } as any, runtimeManager: runtimeManager as any, providerStore: { materializePiRuntime: async () => ({ agentDir: "/tmp", env: {}, providerId: "provider" }) } as any, codexClient: codexClient as any, claudeClient: claudeClient as any, piClient: piClient as any, loadModels: async () => ({ models: [{ model: "model", isDefault: true, inputModalities }] }), resolveProject: async () => ({ id: "project", name: "project", path: process.cwd(), threads: [] }), sendWithCodex: vi.fn(async () => ({ text: "codex" })), sendWithCustomProvider: vi.fn(customSend) });
...
describe("agent runtime IPC", () => {
  it("rejects image input for a text-only model before starting the agent", async () => {
    const test = harness("codex", undefined, ["text"]);
    await expect(test.invoke("agent:start", { runId: "run", agentId: "codex", model: "model", prompt: "hello", images: ["/tmp/image.png"] })).rejects.toThrow("不支持图片");
    expect(test.codexClient.startTurn).not.toHaveBeenCalled();
  });

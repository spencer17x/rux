...
export class GitService {
  ...
  async commitPush(projectId: string, message: string, push: boolean): Promise<GitState> { const project = await this.resolveProject(projectId); if (message) { const staged = await this.runProcess(this.gitExecutable(), ["diff", "--cached", "--quiet"], { cwd: project.path, timeoutMs: 30_000 }); if (staged.code === 0) throw new Error("没有已暂存的变更，请先在审查页暂存文件"); await this.runGit(project.path, ["commit", "-m", message]); } if (push) { const remote = await this.remote(projectId); if (!remote) throw new Error("当前项目没有 origin 远程仓库"); const branch = (await this.runGit(project.path, ["branch", "--show-current"])).trim(); if (!branch) throw new Error("当前不在可推送的本地分支上"); await this.runGit(project.path, ["push", "-u", "origin", branch]); } return await this.status(projectId); }

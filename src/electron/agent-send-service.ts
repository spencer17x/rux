...
export class AgentSendService {
  ...
  async custom(input: CustomSendInput, override?: RuxSettings): Promise<{ text: string }> {
    ...
    for (const path of (input.images || []).slice(0, 8)) {
      try {
        const data = await readFile(resolve(path)); if (data.length > 20 * 1024 * 1024) continue;
        if (/\.(png|jpe?g|gif|webp)$/i.test(path)) { const extension = path.split(".").pop()?.toLowerCase(); const mimeType = extension === "png" ? "image/png" : extension === "gif" ? "image/gif" : extension === "webp" ? "image/webp" : "image/jpeg"; content.push({ type: "input_image", image_url: `data:${mimeType};base64,${data.toString("base64")}` }); }
        else if (data.length <= 512_000 && !data.subarray(0, 8_000).includes(0)) content.push({ type: "input_text", text: `\n\n上下文文件 ${path}:\n${data.toString("utf8")}` });
      } catch {}
    }

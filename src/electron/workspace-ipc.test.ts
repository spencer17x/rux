...
function harness(initial: StoredWorkspace) {
  ...
  registerWorkspaceIpc(ipc, { getWindow: () => null, loadWorkspace: async () => structuredClone(workspace), saveWorkspace: async (next) => { workspace = structuredClone(next); database.saveWorkspace(next); }, stateDatabase: () => database, removeNativeData: async (thread) => { removedNative.push(thread.id); return [thread.nativeSessionId || ""]; }, runProcess: async () => ({ stdout: "", stderr: "", code: 0 }), gitExecutable: () => "git" });
  const invoke = async (channel: string, value?: unknown) => await handlers.get(channel)!({} as any, value);
  return { database, invoke, current: () => workspace, removedNative };
}

describe("workspace IPC", () => {
  it("adds, updates, and removes project threads", async () => {
    const { database, invoke, current, removedNative } = harness({ projects: [{ id: "project", name: "Project", path: "/tmp/project", threads: [] }], standaloneThreads: [] });
    ...
    expect(removedNative).toContain(thread.id);
  });

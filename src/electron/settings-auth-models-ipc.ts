...
export class SettingsAuthModelsIpc {
  ...
  private async models(input?: { agentId?: string; projectId?: string }): Promise<{ models: any[] }> {
    const d = this.deps; if (process.env.RUX_E2E === "1") return input?.agentId && input.agentId !== "codex" ? { models: [] } : { models: [{ id: "e2e-model", model: "e2e-model", displayName: "E2E Model", description: "Deterministic test model", hidden: false, isDefault: true, defaultReasoningEffort: "medium", supportedReasoningEfforts: ["low", "medium", "high"].map(reasoningEffort => ({ reasoningEffort, description: "E2E" })), serviceTiers: [{ id: "priority", name: "Fast", description: "E2E fast mode" }], defaultServiceTier: null }, { id: "gpt-6-astra", model: "gpt-6-astra", displayName: "GPT-6 Astra", description: "Deterministic Astra test model", hidden: false, isDefault: false, defaultReasoningEffort: "high", supportedReasoningEfforts: ["low", "medium", "high", "xhigh", "max", "ultra"].map(reasoningEffort => ({ reasoningEffort, description: "E2E" })) }] };

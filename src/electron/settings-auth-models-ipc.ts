...
export class SettingsAuthModelsIpc {
  ...
  register(ipc: IpcRegistrar): void {
    ...
    ipc.handle("auth:login", async () => { await d.runtimeManager.ensure("codex"); if (this.authLoginProcess && !this.authLoginProcess.killed) return { started: true, alreadyRunning: true }; const child = spawn(d.codexExecutable(), ["login", "--device-auth"], { env: { ...process.env, NO_COLOR: "1" }, stdio: ["pipe", "pipe", "pipe"] }); this.authLoginProcess = child; child.stdin.end(); const emit = (event: Record<string, unknown>) => d.getWindow()?.webContents.send("auth:login-event", event); const output = (chunk: Buffer | string) => { const text = String(chunk).replace(/\u001b\[[0-9;]*m/g, "").trim(); if (text) emit({ type: "output", text }); }; child.stdout.on("data", output); child.stderr.on("data", output); child.on("error", (error) => emit({ type: "error", message: error.message })); child.on("close", (code) => { this.authLoginProcess = null; emit({ type: "complete", code: code ?? 1 }); }); return { started: true }; });
    ipc.handle("auth:logout", async () => { await d.runtimeManager.ensure("codex"); const result = await d.runProcess(d.codexExecutable(), ["logout"], { timeoutMs: 20_000 }); if (result.code !== 0) throw new Error(result.stderr.trim() || "退出登录失败"); return { connected: false }; });

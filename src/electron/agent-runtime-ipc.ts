...
export function registerAgentRuntimeIpc(ipc: IpcRegistrar, deps: Dependencies): void {
  ipc.handle("agent:send", async (_event, value) => { const input = parseInput(agentSendSchema, value); const settings = await deps.settingsStore.load(); return settings.provider === "custom" ? await deps.sendWithCustomProvider(input) : await deps.sendWithCodex(input); });

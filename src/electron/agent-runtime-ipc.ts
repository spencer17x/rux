...
export function registerAgentRuntimeIpc(ipc: IpcRegistrar, deps: Dependencies): void {
  ...
  ipc.handle("agent:start", async (_event, value) => {
    ...
    if (process.env.RUX_E2E === "1" && !custom) {
      ...
      queueMicrotask(() => {
        emit({ type: "text-delta", itemId: `text-${input.runId}`, delta: "RUX_E2E_AGENT_OK" });
        setTimeout(() => emit({ type: "turn-completed", status: "completed", turnId: input.runId, turnInfo: { elapsedMs: 750, usage: input.model === "gpt-6-astra" ? { inputTokens: 9860, outputTokens: 2620, cachedInputTokens: 6144, reasoningOutputTokens: 1820, totalTokens: 12480 } : { inputTokens: 1240, outputTokens: 320, totalTokens: 1560 } } }), 750);
      });
      return { runId: input.runId, threadId: "e2e-thread", turnId: "e2e-turn" };
    }
    if (settings.provider === "custom" && (!input.agentId || input.agentId === "codex")) { const controller = new AbortController(); customRuns.set(input.runId, controller); try { const result = await deps.sendWithCustomProvider({ ...input, signal: controller.signal }); queueMicrotask(() => { const window = deps.getWindow(); window?.webContents.send("agent:event", { runId: input.runId, type: "text-delta", itemId: `text-${input.runId}`, delta: result.text }); window?.webContents.send("agent:event", { runId: input.runId, type: "turn-completed", status: "completed", turnInfo: { agentId: "api", model: result.model || input.model || settings.model, reasoning: input.reasoning || settings.reasoning, usage: result.usage } }); }); return { runId: input.runId, threadId: input.threadId || "" }; } catch (error) { if (controller.signal.aborted) { queueMicrotask(() => deps.getWindow()?.webContents.send("agent:event", { runId: input.runId, type: "turn-completed", status: "interrupted" })); return { runId: input.runId, threadId: input.threadId || "" }; } throw error; } finally { customRuns.delete(input.runId); } }
    ...
    await deps.runtimeManager.ensure("codex");
    try { return await withCodexAuth(() => deps.codexClient.startTurn({ runId: input.runId, threadId: input.threadId, cwd, prompt: input.prompt.trim(), model: input.model || settings.model, reasoning: input.reasoning || settings.reasoning, serviceTier: input.serviceTier, sandboxMode: input.sandboxMode || settings.sandboxMode, images: input.images, webSearch: input.webSearch, mode: input.mode === "plan" ? "plan" : "default" })); } catch (error) { await deps.onCodexAuthError?.(error); throw error; }
  });

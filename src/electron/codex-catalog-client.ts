...
export class CodexCatalogClient {
  ...
  stop(): void { this.generation++; for (const child of this.children) child.kill("SIGTERM"); this.children.clear(); }
  ...
  async account(): Promise<{ connected: boolean; account: CodexAccount | null; message: string }> { try { const result = await this.request<{ account?: CodexAccount | null }>("account/read", { refreshToken: false }); const account = result.account ?? null; return { connected: Boolean(account), account, message: account?.email || account?.type || "" }; } catch (error) { if (isCodexAuthError(error)) throw error; const status = await this.runProcess(this.executable(), ["login", "status"], { timeoutMs: 20_000, env: this.environment() }); return { connected: status.code === 0, account: null, message: (status.stdout || status.stderr || String(error)).trim() }; } }
  async request<T>(method: string, params: unknown): Promise<T> {
    const generation = this.generation;
    await this.runtimeManager.ensure("codex");
    if (generation !== this.generation) throw new Error("Codex 账户已变更，请重试。");
    return await new Promise((resolve, reject) => {
      const child = spawn(this.executable(), ["app-server", "--stdio"], { env: { ...process.env, ...this.environment(), NO_COLOR: "1" }, stdio: ["pipe", "pipe", "pipe"] }); let stdout = ""; let settled = false; this.children.add(child);
      const finish = (error?: Error, result?: T) => { if (settled) return; settled = true; this.children.delete(child); clearTimeout(timeout); child.kill("SIGTERM"); error ? reject(error) : resolve(result as T); };

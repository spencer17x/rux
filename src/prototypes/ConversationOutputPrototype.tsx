...
function Disclosure({ icon, label, meta, open, onToggle, tone = "normal", children }: DisclosureProps) {
  ...
  return <section className={`prototype-tool ${tone} ${open ? "is-open" : ""}`}>
    <button type="button" className="prototype-tool-summary" onClick={onToggle} aria-expanded={open}>
      <Icon size="sm" variant={tone === "error" ? "solid" : "outline"} />
      <span>{label}</span>{meta && <small>{meta}</small>}{open ? <CaretDown size="sm" /> : <CaretRight size="sm" />}
    </button>
...
export default function ConversationOutputPrototype() {
  ...
  return <main className={`conversation-prototype ${paused ? "is-paused" : ""}`}>
    <header className="prototype-topbar">
      <div className="prototype-project"><Folder size="md" /><span>rux-demo</span><i>/</i><strong>优化对话输出体验</strong><button type="button" aria-label="更多">•••</button></div>
      <div className="prototype-top-actions"><button type="button" aria-label="分享"><ShareNetwork size="sm" /></button><button type="button" className="prototype-location">打开位置 <CaretDown size="xs" /></button><button type="button" aria-label="环境信息"><ListBullets size="md" /></button><button type="button" aria-label="底部面板"><SidebarSimple size="md" /></button><button type="button" aria-label="侧栏"><SidebarSimple size="md" className="prototype-sidebar-icon" /></button><button type="button" aria-label="设置"><GearSix size="md" /></button></div>
    </header>

    <section className="prototype-thread">
      <time className="prototype-date">今天 10:18</time>
      <div className="prototype-user-wrap"><div className="prototype-user">把 Rux 的对话输出改得像 Codex 一样自然，工具细节默认折叠。</div><small>10:18 <Copy size="sm" /></small></div>
      ...
      <section className="prototype-turn">
        ...
        <div className="prototype-answer">
          ...
          <Disclosure icon="terminal" label="运行 pnpm test" meta={paused ? "已暂停" : "18.4秒"} open={commandOpen} onToggle={() => setCommandOpen((value) => !value)} tone={paused ? "normal" : "running"}>
            <div className="prototype-command-head"><span><b>命令</b> pnpm test --filter=workbench</span><button type="button" aria-label="复制命令"><Copy size="sm" /></button></div>
          ...
          <Disclosure icon="error" label={retryState === "success" ? "测试失败后已重试成功" : retryState === "retrying" ? "正在重试失败的测试" : "测试失败 · 1 个错误"} meta={retryState === "success" ? "已恢复" : "2.1秒"} open={errorOpen} onToggle={() => setErrorOpen((value) => !value)} tone={retryState === "success" ? "success" : retryState === "retrying" ? "running" : "error"}>
            <div className="prototype-error-detail"><strong>{retryState === "retrying" ? "正在重新运行失败用例…" : "命令执行失败"}</strong><p>测试用例期望返回的模型列表不为空。</p><div><button type="button">查看详情</button><button type="button" className="prototype-retry" onClick={retry}>{retryState === "retrying" ? <><CircleNotch size="sm" className="prototype-spin" />重试中</> : "重试"}</button></div></div>
          </Disclosure>

          {retryState === "success" && <div className="prototype-resumed"><CheckCircle size="sm" variant="solid" />已从失败处继续</div>}
        ...
        <footer className="prototype-turn-footer"><button type="button">用时 38秒 <CaretRight size="xs" /></button><span /><button type="button" aria-label="复制回答"><Copy size="sm" /></button><button type="button" aria-label="赞"><ThumbsUp size="sm" /></button><button type="button" aria-label="踩"><ThumbsDown size="sm" /></button></footer>
      </section>
    </section>

    <section className="prototype-composer">
      <span className="prototype-placeholder">向 Rux 发送消息</span>
      <div className="prototype-composer-row"><div><button type="button" aria-label="添加">＋</button><button type="button" aria-label="联网"><Globe size="sm" /></button><button type="button" className="prototype-permission"><HandPalm size="sm" />请求批准 <CaretDown size="xs" /></button></div><div><button type="button" className="prototype-model">Codex <CaretDown size="xs" /></button><button type="button" className="prototype-model">默认 <CaretDown size="xs" /></button><button type="button" className="prototype-model">GPT-5.6 Sol <CaretDown size="xs" /></button><button type="button" className="prototype-model">中 <CaretDown size="xs" /></button><button type="button" aria-label="语音"><Microphone size="sm" /></button>{paused ? <><button type="button" className="prototype-stop" onClick={reset}><Stop size="xs" variant="solid" />结束</button><button type="button" className="prototype-continue" onClick={() => setPaused(false)}>继续</button></> : <button type="button" className="prototype-pause" onClick={() => setPaused(true)}><Pause size="sm" variant="solid" />暂停</button>}<button type="button" className="prototype-send" aria-label="发送"><ArrowUp size="sm" variant="strong" /></button></div></div>
    </section>

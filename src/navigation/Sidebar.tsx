...
import { CaretDown, CaretRight, ChatCircle, CircleNotch, Folder, FolderOpen, GearSix, MagnifyingGlass, Paperclip, PencilSimple, Plus, Trash } from "../ui/icons";
...
function ThreadRow({ thread, active, child = false, running = false, onSelect, onRename, onDelete }: { thread: ThreadRecord; active: boolean; child?: boolean; running?: boolean; onSelect: () => void; onRename: () => void; onDelete: () => void }) {
  ...
  const row = <div className={`thread-row-wrap ${child ? "is-child" : ""} ${running ? "is-running" : ""}`}>
    <NavItem size="lg" active={active} className={`sidebar-row ${child ? "child-row" : ""} ${active ? "is-selected" : ""}`} title="双击重命名会话" onClick={onSelect} onDoubleClick={(event) => { event.preventDefault(); onRename(); }}><ChatCircle size="md" /><span>{thread.title}</span></NavItem>
...
function ProjectNode({ project, actions, query }: { project: ProjectRecord; actions: Props; query: string }) {
  ...
  return <div className={`project-node ${expanded ? "is-expanded" : ""} ${active ? "has-active-thread" : ""} ${open ? "is-menu-open" : ""}`}>
    <ContextActions label={`项目操作 ${project.name}`} trigger={<div className="project-row-wrap">
      <NavItem size="lg" className="project-row" aria-expanded={expanded} onClick={() => actions.onToggleProject(project.id)}>{expanded ? <CaretDown size="sm" /> : <CaretRight size="sm" />}<Folder size="md" /><span>{project.name}</span></NavItem>
...
export default function Sidebar(props: Props) {
  ...
  return <aside className="sidebar" aria-label="Rux 导航" style={{ flexBasis: sidebarWidth, width: sidebarWidth }}>
    <div className="sidebar-brand-row"><strong className="brand"><img src={ruxMark} alt="" />Rux</strong><div className="sidebar-actions"><IconButton label="搜索" icon="search" size="sm" iconSize="md" active={searchOpen} onClick={() => { setSearchOpen((open) => !open); setProfileOpen(false); }} /></div></div>

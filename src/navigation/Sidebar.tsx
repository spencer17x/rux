...
function ThreadRow({ thread, active, child = false, onSelect, onRename }: { thread: ThreadRecord; active: boolean; child?: boolean; onSelect: () => void; onRename: () => void }) {
  return <div className={`thread-row-wrap ${child ? "is-child" : ""}`}>
    ...
    <IconButton label={`重命名会话 ${thread.title}`} className="thread-rename-button" onClick={(event) => { event.stopPropagation(); onRename(); }}><PencilSimple size={14} /></IconButton>
  </div>;
}
...
export default function Sidebar(props: Props) {
  ...
  return <aside className="sidebar" aria-label="Rux 导航">
    ...
    <nav className="sidebar-scroll"><section className="sidebar-section"><div className="section-heading"><span>独立会话</span><IconButton label="新建独立会话" onClick={props.onNewStandalone}><Plus size={17} /></IconButton></div><div className="sidebar-list">{standaloneThreads.map((thread) => <ThreadRow key={thread.id} thread={thread} active={props.activeThread?.type === "standalone" && props.activeThread.id === thread.id} onSelect={() => props.onSelectStandalone(thread)} onRename={() => props.onRenameThread({ type: "standalone", ...thread })} />)}</div></section>
      <div className="section-divider" />
      <section className="sidebar-section project-section"><div className="section-heading"><span>项目</span><IconButton label="添加项目" onClick={(event) => props.onAddProject(event.currentTarget)}><Plus size={17} /></IconButton></div><div className="project-tree">{projects.map((project) => { const expanded = props.expandedProjects.includes(project.id); return <div className="project-node" data-project-menu-scope={project.id} key={project.id}><div className="project-row-wrap"><button type="button" className="project-row" onClick={() => { props.onToggleProject(project.id); setProjectMenuId(null); }} onDoubleClick={() => props.onOpenProjectPath(project)} onContextMenu={(event) => { event.preventDefault(); setProjectMenuId(project.id); }}>{expanded ? <CaretDown size={14} /> : <CaretRight size={14} />}<Folder size={18} /><span>{project.name}</span></button><IconButton ref={(element) => { projectMenuTriggers.current[project.id] = element; }} label={`项目操作 ${project.name}`} className="project-action-button" active={projectMenuId === project.id} onClick={(event) => { event.stopPropagation(); setProjectMenuId((current) => current === project.id ? null : project.id); }}><DotsThree size={17} /></IconButton></div>
        ...
        {expanded && <div className="thread-children">{project.threads.map((thread) => <ThreadRow key={thread.id} thread={thread} child active={props.activeThread?.type === "project" && props.activeThread.id === thread.id} onSelect={() => props.onSelectProjectThread(project, thread)} onRename={() => props.onRenameThread({ type: "project", projectId: project.id, projectName: project.name, projectPath: project.path, ...thread })} />)}<button type="button" className="sidebar-row child-row new-thread-row" onClick={() => props.onNewProjectThread(project)}><Plus size={16} /><span>新建项目会话</span></button></div>}
      </div>; })}</div></section></nav>

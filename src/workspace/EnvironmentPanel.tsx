...
import { CaretDown, File, FileText, GitBranch, GitCommit, GithubLogo, HardDrive, ListBullets, Paperclip, Plus } from "@phosphor-icons/react";
...
export default function EnvironmentPanel(props: Props) {
  ...
  return <aside className="environment-panel" aria-label="环境信息">
    ...
    <div className="environment-menu-wrap"><button type="button" className="environment-row" disabled={disabled || props.busy || props.readOnly || props.gitState.branch === "—"} aria-expanded={branchOpen} onClick={() => { setBranchOpen((open) => !open); setCompareOpen(false); }}><GitBranch size={18} /><span>{props.gitState.branch || "—"}</span><CaretDown className="row-end" size={15} /></button>{branchOpen && <div className="branch-popover" role="menu">{props.branches.length ? props.branches.map((branch) => <button type="button" role="menuitem" className={branch === props.gitState.branch ? "is-selected" : ""} key={branch} onClick={() => { setBranchOpen(false); props.onSwitchBranch(branch); }}>{branch}</button>) : <span>没有可切换的本地分支</span>}</div>}</div>
    <button type="button" className={`environment-row ${disabled || props.gitState.branch === "—" ? "is-disabled" : ""}`} disabled={disabled || props.gitState.branch === "—" || props.busy || props.readOnly} onClick={props.onCommitPush}><GitCommit size={18} /><span>提交或推送</span></button>
    <div className="environment-menu-wrap"><button type="button" className="environment-row" disabled={disabled || props.gitState.branch === "—" || props.branches.filter((branch) => branch !== props.gitState.branch).length === 0} aria-expanded={compareOpen} onClick={() => { setCompareOpen((open) => !open); setBranchOpen(false); }}><GithubLogo size={18} /><span>比较分支</span><CaretDown className="row-end" size={15} /></button>{compareOpen && <div className="branch-popover" role="menu">{props.branches.filter((branch) => branch !== props.gitState.branch).map((branch) => <button type="button" role="menuitem" key={branch} onClick={() => { setCompareOpen(false); props.onCompareBranch(branch); }}>{branch}</button>)}</div>}</div>

...
type Props = {
  ...
  terminalProps: { output: TerminalChunk[]; onInput: (data: string) => void; onResize: (size: { cols: number; rows: number }) => void };
...
export default function WorkspaceDock(props: Props) {
  ...
  return <section className="workspace-dock" aria-label="底部工作区面板">
    <header className="workspace-dock-header"><div className="workspace-dock-tabs">{tools.map(({ id, label, Icon, projectOnly }) => <button type="button" key={id} className={activeTool === id ? "is-active" : ""} disabled={projectOnly && !hasProject} onClick={() => props.onSelectTool(id)}><Icon size={15} />{label}</button>)}</div><button type="button" className="icon-button" aria-label="关闭底部面板" onClick={props.onClose}><X size={16} /></button></header>
    <div className="workspace-dock-content">
      ...
      {activeTool === "chat" && <div className="dock-side-chat"><div className="dock-chat-messages">{sideMessages.length ? sideMessages.map((message) => <p key={message.id} className={message.role === "user" ? "is-user" : "is-agent"}>{message.text}</p>) : <span>针对当前工作区快速提问，不影响主会话。</span>}</div><form onSubmit={(event) => { event.preventDefault(); props.onSendSide(); }}><input aria-label="侧边聊天消息" value={sideValue} onChange={(event) => props.onSideValue(event.target.value)} disabled={sideSending} /><button type="submit" aria-label="发送侧边聊天消息" disabled={sideSending || !sideValue.trim()}>{sideSending ? <CircleNotch size={15} className="spin" /> : <ArrowUp size={15} />}</button></form></div>}
    </div>
  </section>;
}

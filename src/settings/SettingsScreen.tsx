...
export default function SettingsScreen(props: Props) {
  const { settings, auth, models, modelsLoading, modelsError, agents, modelsByAgent, providerStore, systemInfo, projectCount, activeProject, gitState, onBack, onSave, onTest, onLogin, onLogout, onNotify } = props;
  ...
  return <div className="settings-shell">
    ...
    <main className="settings-content"><h1>{sections.find(([id]) => id === section)?.[1] || "设置"}</h1>
      ...
      {section === "permissions" && <section className="settings-section"><h2>默认权限</h2><div className="settings-group form-settings"><div className="settings-row"><span>Codex 沙盒</span><div className="reasoning-control" role="group" aria-label="Codex 沙盒">{Object.entries(sandboxLabels).map(([value, label]) => <button type="button" key={value} aria-pressed={draft.sandboxMode === value} className={draft.sandboxMode === value ? "is-selected" : ""} onClick={() => update({ sandboxMode: value as SandboxMode })}>{label}</button>)}</div></div><div className="settings-row settings-actions"><button type="button" className="primary-button" onClick={() => void execute(() => saveDraft("默认权限已保存"))}>保存权限</button></div></div></section>}

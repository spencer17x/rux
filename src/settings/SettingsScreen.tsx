...
type Settings = {
  ...
  model: string; reasoning: Reasoning; sandboxMode: SandboxMode; uiFontSize: number;
};
...
type Props = {
  settings: Settings; auth: AuthState; models: Model[]; agents: Agent[]; modelsByAgent: Record<string, Model[]>;
  ...
  onBack: () => void; onSave: AsyncAction; onTest: AsyncAction; onLogin: AsyncAction; onLogout: AsyncAction; onNotify: (message: string) => void;
};
...
function ProviderProfiles({ store, onSave, onRemove, onSetActive, onTest, onNotify }: { store: ProviderStore; onSave: AsyncAction; onRemove: AsyncAction; onSetActive: AsyncAction; onTest: AsyncAction; onNotify: (message: string) => void }) {
  ...
  const execute = async (action: () => Promise<string | void>) => { setBusy(true); setStatus(""); try { setStatus((await action()) || ""); } catch (error) { setStatus(error instanceof Error ? error.message : String(error)); } finally { setBusy(false); } };
  ...
  return <div className="provider-settings-layout">
    ...
    </div>{status && <p className={/失败|错误/.test(status) ? "settings-status error-text" : "settings-status connected"}>{status}</p>}</section>
  </div>;
}

export default function SettingsScreen(props: Props) {
  const { settings, auth, models, agents, modelsByAgent, providerStore, systemInfo, projectCount, activeProject, gitState, onBack, onSave, onTest, onLogin, onLogout, onNotify } = props;
  ...
  return <div className="settings-shell">
    ...
    <main className="settings-content"><h1>{sections.find(([id]) => id === section)?.[1] || "设置"}</h1>
      ...
      {section === "agents" && <section className="settings-section"><h2>底座 Agent</h2><div className="agent-settings-list">{agents.map((agent) => <article className="agent-settings-card" key={agent.id}><span className="agent-settings-icon"><Robot size={20} /></span><span className="agent-settings-copy"><strong>{agent.name}</strong><small>{agent.version || "运行时"} · {agent.installed ? "已下载" : "首次使用时下载"}</small><em>{agent.id === "claude-code" ? (agent.auth?.connected ? `已登录 · ${agent.auth.authMethod || "Claude"}` : "需要登录 Claude") : agent.id === "codex" ? (auth.connected ? "GPT OAuth 已连接" : "需要登录 Codex") : providerStore.profiles.some((profile) => profile.compatibleAgents.includes("pi")) ? "Provider 已配置" : "需要 Pi Provider"}</em></span><span className={`agent-settings-status ${agent.installed && agent.integrated ? "is-ready" : ""}`}>{agent.installed ? "已下载" : "未下载"}</span><div className="agent-settings-modes">{agent.modes?.map((item) => <span key={item.id}>{item.label}</span>)}</div><small className="agent-settings-models">{agent.id === "codex" ? models.length : modelsByAgent[agent.id]?.length || 0} 个可用模型</small></article>)}</div></section>}
      ...
      {section === "permissions" && <section className="settings-section"><h2>默认权限</h2><div className="settings-group form-settings"><div className="settings-row"><span>Codex 沙盒</span><div className="reasoning-control">{Object.entries(sandboxLabels).map(([value, label]) => <button type="button" key={value} className={draft.sandboxMode === value ? "is-selected" : ""} onClick={() => update({ sandboxMode: value as SandboxMode })}>{label}</button>)}</div></div><div className="settings-row settings-actions"><button type="button" className="primary-button" onClick={() => void execute(() => saveDraft("默认权限已保存"))}>保存权限</button></div></div></section>}
      {section === "models" && <><section className="settings-section"><h2>GPT OAuth</h2><div className="settings-group oauth-group"><div className="settings-row provider-row"><div className="provider-mark"><OpenAiLogo size={27} /></div><strong>Codex CLI</strong><span className={auth.connected ? "connected-badge" : "disconnected-badge"}>{auth.connected ? "已连接" : "未登录"}</span><span className="provider-email">{auth.account?.email || auth.message || "未检测到登录"}</span><button type="button" className="secondary-button" onClick={() => void execute(async () => { await onLogin(); return "设备登录已启动"; })}>{auth.connected ? "重新登录" : "登录"}</button></div>{auth.connected && <button type="button" className="danger-link disconnect-link" onClick={() => { if (window.confirm("确认退出本机 Codex 登录？")) void execute(onLogout); }}>断开连接</button>}</div></section><section className="settings-section"><h2>连接方式</h2><div className="settings-group"><div className="settings-row provider-choice"><button type="button" className={draft.provider === "codex" ? "is-selected" : ""} onClick={() => update({ provider: "codex" })}>GPT OAuth</button><button type="button" className={draft.provider === "custom" ? "is-selected" : ""} onClick={() => update({ provider: "custom" })}>自定义服务</button></div></div></section>{draft.provider === "custom" && <section className="settings-section"><h2>自定义服务</h2><div className="settings-group form-settings"><label className="settings-row"><span>服务名称</span><input value={draft.serviceName} onChange={(event) => update({ serviceName: event.target.value })} /></label><label className="settings-row"><span>Base URL</span><input value={draft.baseUrl} onChange={(event) => update({ baseUrl: event.target.value })} /></label><label className="settings-row"><span>API key</span><span className="secret-input"><input type="password" value={apiKey} placeholder={draft.hasApiKey ? "已安全保存" : "输入 API key"} onChange={(event) => setApiKey(event.target.value)} /><Eye size={18} /></span></label><div className="settings-row settings-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => void execute(async () => (await onTest({ ...draft, apiKey })).message)}>测试连接</button><button type="button" className="primary-button" disabled={busy} onClick={() => void execute(async () => { await onSave({ ...draft, apiKey }); setApiKey(""); return "已保存"; })}>保存服务</button></div></div></section>}<section className="settings-section"><h2>默认模型</h2><div className="settings-group form-settings"><label className="settings-row"><span>模型</span>{draft.provider === "codex" ? <select value={model?.model || ""} onChange={(event) => { const next = models.find((item) => item.model === event.target.value); if (next) update({ model: next.model, reasoning: next.supportedReasoningEfforts.some((item) => item.reasoningEffort === draft.reasoning) ? draft.reasoning : next.defaultReasoningEffort }); }}>{models.map((item) => <option key={item.id} value={item.model}>{item.displayName}{item.isDefault ? "（默认）" : ""}</option>)}</select> : <input value={draft.model} onChange={(event) => update({ model: event.target.value })} />}<button type="button" className="secondary-button" onClick={() => update({ model: "", reasoning: models.find((item) => item.isDefault)?.defaultReasoningEffort || "medium" })}><ArrowsClockwise size={16} />使用默认</button></label><div className="settings-row"><span>思考程度</span><div className="reasoning-control">{efforts.map((value) => <button type="button" key={value} className={draft.reasoning === value ? "is-selected" : ""} onClick={() => update({ reasoning: value })}>{reasoningLabels[value]}</button>)}</div></div><div className="settings-row settings-actions"><button type="button" className="primary-button" disabled={busy} onClick={() => void execute(() => saveDraft("默认模型设置已保存"))}>保存默认设置</button></div></div></section></>}

...
export function useTerminalController(api: RuxApi, projectId: string | undefined, onCommandCommitted: () => void, onError: (message: string) => void) {
  ...
  const append = useCallback((data: string) => {
    sequence.current += 1;
    setOutput((current) => [...current, { sequence: sequence.current, data }].slice(-2000));
  }, []);
  useEffect(() => api.terminal.onData((data) => append(String(data))), [api, append]);
  const start = useCallback(async () => {
    if (!projectId || open) return;
    sequence.current += 1;
    setOutput([{ sequence: sequence.current, data: "正在启动终端…\r\n" }]);
    setOpen(true);
    startTask.current = api.terminal.start(projectId).then(() => api.terminal.resize(size.current));
    try { await startTask.current; }
    catch (error) { append(`${error instanceof Error ? error.message : String(error)}\r\n`); }
  }, [api, append, open, projectId]);
  const close = useCallback(async () => { if (open) await api.terminal.stop().catch(() => {}); setOpen(false); }, [api, open]);
  useEffect(() => {
  ...
  return { terminalOpen: open, terminalOutput: output, appendTerminalOutput: append, startTerminal: start, closeTerminal: close, writeTerminalInput: write, resizeTerminal: resize };
}

...
type SideRun = { runId: string; assistantId: string; agentId: AgentId; threadId: string; turnId: string; deltaItems: Set<string>; abandoned: boolean };
...
export function useWorkspaceTools(api: RuxApi, projectId: string | undefined, selectedAgent: AgentId, agentMode: string, preference: { model: string; reasoning: AppSettings["reasoning"]; serviceTier: string | null }, settings: AppSettings, refreshGit: (projectId?: string) => Promise<void>, notify: (message: string) => void) {
  ...
  useEffect(() => {
    setSideMessages([]); setSideThreadId(""); setSideValue(""); setSideApproval(null); setSideSending(false);
    return () => {
      const active = sideRun.current;
      sideRun.current = null;
      if (!active) return;
      active.abandoned = true;
      void api.agent.interrupt({ agentId: active.agentId, runId: active.runId, threadId: active.threadId || active.runId, turnId: active.turnId || active.runId }).catch(() => {});
    };
  }, [api, projectId, selectedAgent]);
  useEffect(() => { setProjectFiles([]); setRemoteUrl(""); }, [projectId]);
  useEffect(() => {
    if (!bottomPanelOpen && !rightPanelOpen) return;
    if (!projectId && projectOnlyTools.has(activeTool)) { setActiveTool("environment"); return; }
    let cancelled = false;
    if (activeTool === "terminal") void terminal.startTerminal();
    if (projectId && activeTool === "files") api.files.list(projectId).then((files) => { if (!cancelled) setProjectFiles(files as string[]); }).catch((error) => { if (!cancelled) notify(userFacingError(error)); });
    if (projectId && activeTool === "browser") api.git.remote(projectId).then((url) => { if (!cancelled) setRemoteUrl(String(url)); }).catch((error) => { if (!cancelled) notify(userFacingError(error)); });
    return () => { cancelled = true; };
  }, [activeTool, api, bottomPanelOpen, notify, projectId, rightPanelOpen, terminal.startTerminal]);
  ...
  const closeBottomPanel = useCallback(async () => { setBottomPanelOpen(false); }, []);
  const closeRightPanel = useCallback(async () => { setRightPanelOpen(false); }, []);
  ...
  const sendSideChat = useCallback(async () => {
    const prompt = sideValue.trim(); if (!prompt || sideSending || sideRun.current) return;
    ...
    const run: SideRun = { runId, assistantId, agentId: selectedAgent, threadId: sideThreadId, turnId: "", deltaItems: new Set(), abandoned: false };
    sideRun.current = run;
    ...
    try {
      ...
      run.threadId = result.threadId || result.sessionId || run.threadId;
      run.turnId = result.turnId || run.turnId;
      if (run.abandoned) {
        // Early cancellation may have preceded native session creation.
        await api.agent.interrupt({ agentId: run.agentId, runId, threadId: run.threadId || runId, turnId: run.turnId || runId });
      } else if (sideRun.current === run && run.threadId) setSideThreadId(run.threadId);
    } catch (error) {
      if (sideRun.current?.runId !== runId) return;
  ...
  const respondToSideApproval = useCallback(async (decision: "accept" | "acceptForSession" | "decline") => {
    const run = sideRun.current;
    if (!sideApproval || !run) return;
    try {
      await api.agent.respondToApproval({ approvalId: sideApproval.id, decision });
      if (sideRun.current === run) setSideApproval((current) => current?.id === sideApproval.id ? null : current);
    } catch (error) { if (sideRun.current === run) notify(userFacingError(error)); }
  }, [api, notify, sideApproval]);

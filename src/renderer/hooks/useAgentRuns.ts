...
export function useAgentRuns(input: Input) {
  ...
  const sendMessage = useCallback(async (nextPrompt = "") => {
    ...
    try { const result = await input.api.agent.start({ runId, agentId: input.selectedAgent, projectId: input.activeProjectId, prompt, model: input.preference.model, reasoning: input.preference.reasoning, sandboxMode: input.settings.sandboxMode, images: input.attachments, webSearch: input.webSearch, ...(nativeSessionId ? { threadId: nativeSessionId, nativeSessionId } : {}), mode: input.agentMode }) as Record<string, string>; context.threadId = result.threadId || result.sessionId || context.threadId; context.turnId = result.turnId || context.turnId; input.setAttachments([]); }

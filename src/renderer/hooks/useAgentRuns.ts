...
export function useAgentRuns(input: Input) {
  ...
  const sendMessage = useCallback(async (nextPrompt = "") => {
    ...
    const nativeSessionId = input.activeThread.nativeSessionId || (input.selectedAgent === "codex" ? input.activeThread.codexThreadId : "") || "";
    const context: RunContext = { runId, localThreadId: input.activeThread.id, assistantMessageId: assistantMessage.id, type: input.activeThread.type, projectId: input.activeThread.projectId, prompt, shouldRename: input.activeThread.title.startsWith("未命名") || input.activeThread.title === "项目会话", agentId: input.selectedAgent, agentMode: input.agentMode, threadId: nativeSessionId, turnId: "" };
    contexts.current.set(runId, context); input.setMessages((current) => ({ ...current, [input.activeThread!.id]: [...(current[input.activeThread!.id] || []), userMessage, assistantMessage] })); input.setComposerValue(""); setRunningThreadIds((current) => new Set(current).add(input.activeThread!.id));
    try { const result = await input.api.agent.start({ runId, agentId: input.selectedAgent, projectId: input.activeProjectId, prompt, model: input.preference.model, reasoning: input.preference.reasoning, sandboxMode: input.settings.sandboxMode, images: input.attachments, webSearch: input.webSearch, threadId: nativeSessionId, nativeSessionId, mode: input.agentMode }) as Record<string, string>; context.threadId = result.threadId || result.sessionId || context.threadId; context.turnId = result.turnId || context.turnId; input.setAttachments([]); }
    catch (error) { contexts.current.delete(runId); input.setMessages((current) => ({ ...current, [input.activeThread!.id]: (current[input.activeThread!.id] || []).map((message) => message.id === assistantMessage.id ? reduceStreamEvent(message, { type: "error", error: error instanceof Error ? error.message : String(error) }) : message) })); setRunningThreadIds((current) => { const next = new Set(current); next.delete(input.activeThread!.id); return next; }); }
  }, [input, sending]);
  ...
  return { sending, sendMessage, cancelCurrentRun, respondToApproval, isProjectRunning: (projectId: string) => [...contexts.current.values()].some((context) => context.projectId === projectId) };
}

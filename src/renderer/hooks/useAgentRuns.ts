...
export function useAgentRuns(input: Input) {
  ...
  return { sending, runningThreadIds, sendMessage, cancelCurrentRun, respondToApproval, isThreadRunning: (threadId: string) => [...contexts.current.values()].some((context) => context.localThreadId === threadId), isProjectRunning: (projectId: string) => [...contexts.current.values()].some((context) => context.projectId === projectId) };
}

...
export function installWebMock(): void {
  ...
  window.rux = {
    ...
    models: { list: async () => ({ models: ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5", "gpt-5.4", "gpt-5.4-mini", "gpt-5.3-codex-spark"].map((model, index) => ({ id: model, model, displayName: model.replace(/^gpt-/i, "GPT-").replace(/-(sol|terra|luna|mini|codex-spark)$/i, (_, name: string) => ` ${name.split("-").map((part) => part[0].toUpperCase() + part.slice(1)).join(" ")}`), description: "本地视觉预览模型", isDefault: index === 0, defaultReasoningEffort: "xhigh", supportedReasoningEfforts: ["low", "medium", "high", "xhigh", "max", "ultra"].map((reasoningEffort) => ({ reasoningEffort, description: reasoningEffort === "ultra" ? "更快消耗使用额度" : "" })), serviceTiers: index < 3 ? [{ id: "priority", name: "Fast", description: "1.5x speed, increased usage" }] : [], defaultServiceTier: null })) }) },
